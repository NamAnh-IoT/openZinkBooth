...
object LogManager {
    ...
    private const val TAG = "openZinkBoothLog"
    private const val LOG_SUB_DIRECTORY = "logs"
    private const val CURRENT_LOG_FILE_NAME_BASE = "openZinkBooth_current_log"
    ...
    private fun writeInitialLogHeaders(logFile: File) {
        try {
            ...
            OutputStreamWriter(
                ...
                writer.append("| App | openZinkBooth |\n")
    ...
    /**
     * Gets the number of log entries in the current log file.
     */
    fun getActualLogEntryCount(): Int {
        val file = getLogFile() ?: return 0
        val logPrefixes = setOf('+', '-', '!', '?', '.')

        return try {
            file.useLines { lines ->
                lines.filter { it.isNotEmpty() && logPrefixes.contains(it[0]) }
                    .drop(1)
                    .count()
            }
        } catch (e: Exception) {
